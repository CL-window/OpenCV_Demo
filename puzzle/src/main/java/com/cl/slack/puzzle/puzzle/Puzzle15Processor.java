...
//
//                int k = i * GRID_SIZE + j;
//                cells[k] = inputPicture.submat(i * rows / GRID_SIZE, (i + 1) * rows / GRID_SIZE, j * cols / GRID_SIZE, (j + 1) * cols / GRID_SIZE);
//
//            }
//        }
    ...
    public void deliverTouchEvent(int x, int y) {
        ...
        int row = (int) Math.floor(x * GRID_SIZE / rows); // 列
        int col = (int) Math.floor(y * GRID_SIZE / cols) - 1; // 行

        StringBuilder builder = new StringBuilder();
        for (int i = 0; i < mIndexes.length; i++) {
            builder.append(mIndexes[i] + " ");
        }

        Log.e(TAG, "row:" + row + " col:" + col + " empty:" + GRID_EMPTY_INDEX + " indexs:" + builder.toString());
        ...
        int idx = (GRID_SIZE - 1) * GRID_SIZE + col - row * GRID_SIZE; // 当前点击的位置对应的 上图数字
        int idxtoswap = -1;
        int dest;
        ...
        if (idxtoswap < 0 && row > 0) {
            dest = idx + GRID_SIZE;
            if (legal(dest) && mIndexes[dest] == GRID_EMPTY_INDEX)
                idxtoswap = dest;
        }
        // right
    ...
    private boolean legal(int index) {
        return index >= 0 && index < GRID_AREA;
    }
